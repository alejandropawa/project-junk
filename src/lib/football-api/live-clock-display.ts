...
function minuteWithAddedTime(minute: number | null, cap: number): string | null {
  if (minute == null) return null;
  if (minute > cap) return `${cap}' +${minute - cap}`;
  return `${minute}'`;
}

function periodClock(prefix: "R1" | "R2" | "PR", minute: number | null, cap: number): string {
  const clock = minuteWithAddedTime(minute, cap);
...
export function liveFixtureClockLabel(f: ClockFields): string | null {
  ...
  if (s === "INPLAY_1ST_HALF" || s === "1ST" || s === "1H") {
    return periodClock("R1", f.minute, 45);
  }
  if (s === "INPLAY_2ND_HALF" || s === "2ND" || s === "2H") {
    return periodClock("R2", f.minute, 90);
  }
  if (s === "INPLAY_ET" || s === "ET") {
    return periodClock("PR", f.minute, 105);
  }
  if (s === "INPLAY_ET_2ND_HALF" || s === "INPLAY_ET_SECOND_HALF" || s === "2ET") {
    return periodClock("PR", f.minute, 120);
  }
