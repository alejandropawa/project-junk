...
/** IDs emise de motor Probix → mapare cote Odds API. */
export function probixAnchoredMarketIds(): Set<string> {
...
export const PROBIX_ODDS_MARKET_IDS = probixAnchoredMarketIds();
...
function addOuIfKnown(
  ...
  if (!PROBIX_ODDS_MARKET_IDS.has(id)) return;
...
export function parseProbixAnchorsFromMarkets(
  ...
  for (const row of markets) {
    ...
    const isFouls =
      /\bfouls?\b|\bfaults?\b/i.test(name) && !isCorners && !isCards;
...
/** Mediană inter–bookmaker pentru fiecare `marketId` (chei Probix). */
export function decimalsPerMarketAcrossBookmakers(
  body: OddsApiOddsBody,
